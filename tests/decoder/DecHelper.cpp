...
std::unique_ptr<DecHelper> DecHelper::Builder::build() {
  ...
  if (mRefMd5Path.has_value()) {
    ...
    while (std::getline(md5File, line)) {
      if (!line.empty()) {
        helper->mRefMd5s.push_back(line);
      }
    }
  }
  ...
  if (mOutFilePath.has_value()) {
    if (!helper->mOutFile.open(*mOutFilePath, false)) {
      return nullptr;
    }
  }
...
bool DecHelper::decodeHeader(size_t& bytesConsumed) {
  ...
  if (ret == IV_SUCCESS) {
    ...
    return setDecoderConfig(mCodec, IVD_DECODE_FRAME, mWidth);
  }
...
bool DecHelper::decodeFrame(size_t& bytesConsumed, bool& frameReady,
  ...
  if (decodeOp.s_ivd_video_decode_op_t.u4_output_present == 1) {
    ...
    // Copy decoded frame from internal reference buffers to our output RawBuf
    size_t planesCount = mOutputBuf.numPlanes();
    for (size_t i = 0; i < planesCount; ++i) {
      const uint8_t* srcPtr = nullptr;
      size_t srcStride = 0;
      if (i == 0) {
        srcPtr = static_cast<const uint8_t*>(dispBuf.pv_y_buf);
        srcStride = dispBuf.u4_y_strd;
      } else if (i == 1) {
        srcPtr = static_cast<const uint8_t*>(dispBuf.pv_u_buf);
        srcStride = dispBuf.u4_u_strd;
      } else if (i == 2) {
        srcPtr = static_cast<const uint8_t*>(dispBuf.pv_v_buf);
        srcStride = dispBuf.u4_v_strd;
      }

      uint8_t* dstPtr = mOutputBuf.planeData(i);
      size_t dstStride = mOutputBuf.stride(i);
      size_t planeW = mOutputBuf.planeWidth(i);
      size_t planeH = mOutputBuf.planeHeight(i);

      if (srcPtr && dstPtr) {
        for (size_t r = 0; r < planeH; ++r) {
          std::memcpy(dstPtr + (r * dstStride), srcPtr + (r * srcStride),
                      planeW);
        }
      }
    }
...
bool DecHelper::decodeFile() {
  ...
  while (!mBitsFile.isEof()) {
    size_t bytesRead = mBitsFile.read(mInputBuf, mInputBuf.capacity());
    if (bytesRead == 0) break;

    size_t bytesConsumed = 0;
    if (!mHeaderDecoded) {
      if (!decodeHeader(bytesConsumed)) return false;
    }

    while (bytesConsumed < mInputBuf.size()) {
      size_t consumed = 0;
      bool frameReady = false;

      mInputBuf.setOffset(bytesConsumed);

                        ...
                        WORD32 log2_trans_size,WORD16 i2_coeff_value)
{
    ...
    LWORD64 shuffle_mask = 0x0E0C0A0806040200;
    LWORD64 unchanged_mask = 0x0F0D0B0907050301;
    ...
    for(row=0; row < trans_size; row++)
        for(col=0; col < trans_size; col++)
            pu1_dst[row * dst_strd + col * col_mult] = CLIP_U8( (pu1_pred[row * pred_strd + col * col_mult] + dc_value));
    */


}
//#endif /* IT_RECON_DC_LUMA */

//#if IT_RECON_DC_CHROMA == X86INTR
...
//#if IT_RECON_DC_CHROMA == ATOMINTR

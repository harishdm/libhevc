...
void ihevcd_sao_ctb(sao_ctxt_t *ps_sao_ctxt)
{
    ...
    pu1_src_luma = ps_sao_ctxt->pu1_cur_pic_luma +
        ((i4_ctb_x + i4_ctb_y * src_strd) << (log2_ctb_size));
    pu1_src_chroma = ps_sao_ctxt->pu1_cur_pic_chroma +
        ((i4_ctb_x + i4_ctb_y * (src_strd >> 1)) << (log2_ctb_size));

    ps_sao = ps_sao_ctxt->ps_pic_sao + i4_ctb_x + i4_ctb_y * ps_sps->i2_pic_wd_in_ctb;
    ...
    /* Current CTB */
    {
        ...
        remaining_cols = ps_sps->i2_pic_width_in_luma_samples - (i4_ctb_x << log2_ctb_size);
        sao_wd_luma = MIN(ctb_size, remaining_cols);
        sao_wd_chroma = MIN(ctb_size, remaining_cols);

        remaining_rows = ps_sps->i2_pic_height_in_luma_samples - (i4_ctb_y << log2_ctb_size);
        sao_ht_luma = MIN(ctb_size, remaining_rows);
        sao_ht_chroma = MIN(ctb_size, remaining_rows) / 2;

        pu1_src_top_luma = ps_sao_ctxt->pu1_sao_src_top_luma + (i4_ctb_x << log2_ctb_size);
        pu1_src_top_chroma = ps_sao_ctxt->pu1_sao_src_top_chroma + (i4_ctb_x << log2_ctb_size);
        pu1_src_left_luma = ps_sao_ctxt->pu1_sao_src_left_luma + (i4_ctb_y << log2_ctb_size);
        pu1_src_left_chroma = ps_sao_ctxt->pu1_sao_src_left_chroma + (i4_ctb_y << log2_ctb_size);

        pu1_no_loop_filter_flag = ps_sao_ctxt->pu1_pic_no_loop_filter_flag +
                ((i4_ctb_y * ctb_size ) / 8) * loop_filter_strd +
                ((i4_ctb_x * ctb_size) / 64);
        ...
        if(ps_sps->i2_pic_wd_in_ctb - 1 == i4_ctb_x)
        {
            au1_avail_luma[1] = 0;
            au1_avail_luma[5] = 0;
            au1_avail_luma[7] = 0;
 ...
 * SAO at CTB level is implemented for a shifted CTB(8 pixels in x and y directions)
 */
void ihevcd_10bd_sao_ctb(sao_ctxt_t *ps_sao_ctxt)
{
    codec_t *ps_codec;
    ...
    /* SaoOffsetVal[] arrays fopr luma and chroma components */
    /* WORD8 only is needed for upto 10-bit, keeping WORD16  */
    /* for the pixel processing functions which are common   */
    /* for 9 to 14 bits */
    WORD8 ai1_offset_y[5];
    WORD8 ai1_offset_cb[5];
    WORD8 ai1_offset_cr[5];
    ...
    pu2_src_luma = (UWORD16 *)ps_sao_ctxt->pu1_cur_pic_luma +
        ((i4_ctb_x + i4_ctb_y * src_strd) << (log2_ctb_size));
    ...
    /* Current CTB */
    {
        ...
        if(0 == ps_sao->b3_y_type_idx)
        {
            ...
            *(UWORD16 *)ps_sao_ctxt->pu1_sao_src_top_left_luma_curr_ctb = pu2_src_top_luma[sao_wd_luma - 1];

            ihevc_memcpy((UWORD8 *)pu2_src_top_luma, (UWORD8 *)&pu2_src_luma[(sao_ht_luma - 1) * src_strd], sao_wd_luma * 2);

        }
        else
        {
            UWORD16 au2_src_copy[(MAX_CTB_SIZE + 2) * (MAX_CTB_SIZE + 2)];
            UWORD16 *pu2_src_copy = au2_src_copy + (MAX_CTB_SIZE + 2) + 1;
            WORD32 tmp_strd = MAX_CTB_SIZE + 2;
            WORD32 no_loop_filter_enabled = 0;
            ...
            /* Check the loop filter flags and copy the original values for back up */
            {
                UWORD32 u4_no_loop_filter_flag;
                WORD32 min_cu = 8;
                UWORD16 *pu2_src_tmp = pu2_src_luma;
                ...
                for(i = 0; i < (sao_ht_luma + min_cu - 1) / min_cu; i++)
                {
                    ...
                    if(u4_no_loop_filter_flag)
                    {
                        ...
                        while(tmp_wd > 0)
                        {
                            ...
                            {
                                u4_no_loop_filter_flag  >>= (CTZ(u4_no_loop_filter_flag));
                                pu2_src_tmp += MIN(CTZ(u4_no_loop_filter_flag), tmp_wd);
                                pu2_src_copy += MIN(CTZ(u4_no_loop_filter_flag), tmp_wd);
                                tmp_wd -= CTZ(u4_no_loop_filter_flag) * min_cu;
                            }
                            else
                            {
                                for(row = 0; row < MIN(min_cu, sao_ht_luma - (i - 1) * min_cu); row++)
                                {
                                    for(col = 0; col < MIN(CTZ(~u4_no_loop_filter_flag) * min_cu, tmp_wd); col ++)
                                    {
                                        pu2_src_copy[row * src_strd + col] = pu2_src_tmp[row * tmp_strd + col];
                                    }
                                }
                        ...
                        pu2_src_tmp -= sao_wd_luma;
                    }

                    pu2_src_tmp += min_cu * src_strd;
                    pu2_src_copy += min_cu * tmp_strd;
                }
            }
            ...
            if(1 == ps_sao->b3_y_type_idx)
            {
                ps_codec->s_func_selector.pf_hbd_sao_bo_luma(pu2_src_luma,
                        ...
                        pu2_src_left_luma,
                        pu2_src_top_luma,
                        (UWORD16 *)ps_sao_ctxt->pu1_sao_src_top_left_luma_curr_ctb,
                        ps_sao->b5_y_band_pos,
                        ai1_offset_y,
                        sao_wd_luma,
                        sao_ht_luma,
            ...
            else // if(2 <= ps_sao->b3_y_type_idx)
            {
                ...
                au2_src_bot_left[0] = pu2_src_luma[sao_ht_luma * src_strd - 1];
            ...
            if(no_loop_filter_enabled)
            {
                ...
                for(i = 0; i < (sao_ht_luma + min_cu - 1) / min_cu; i++)
                {
                    u4_no_loop_filter_flag = (*(UWORD32 *)(pu1_no_loop_filter_flag + i * loop_filter_strd)) >> ((((i4_ctb_x << log2_ctb_size) - sao_wd_luma) / 8) % 8);
                    u4_no_loop_filter_flag &= (1 << ((sao_wd_luma + (min_cu - 1)) / min_cu)) - 1;
                        ...
                        while(tmp_wd > 0)
                        {
                            ...
                            else
                            {
                                for(row = 0; row < MIN(min_cu, sao_ht_luma - (i - 1) * min_cu); row++)
                                {
                                    for(col = 0; col < MIN(CTZ(~u4_no_loop_filter_flag) * min_cu, tmp_wd); col ++)
                                    {
                                        pu2_src_tmp[row * src_strd + col] = pu2_src_copy[row * tmp_strd + col];
                                    }
                                }
        ...
        if(0 == ps_sao->b3_cb_type_idx)
        {
            ...
            *(UWORD16 *)ps_sao_ctxt->pu1_sao_src_top_left_chroma_curr_ctb = pu2_src_top_chroma[sao_wd_chroma - 2];
            *((UWORD16 *)ps_sao_ctxt->pu1_sao_src_top_left_chroma_curr_ctb + 1) = pu2_src_top_chroma[sao_wd_chroma - 1];

            ihevc_memcpy((UWORD8 *)pu2_src_top_chroma, (UWORD8 *)&pu2_src_chroma[(sao_ht_chroma - 1) * src_strd],
                         sao_wd_chroma * sizeof(UWORD16));
        }
        else
        {
            UWORD16 au2_src_copy[(MAX_CTB_SIZE + 4) * (MAX_CTB_SIZE + 2)];
            UWORD16 *pu2_src_copy = au2_src_copy + (MAX_CTB_SIZE + 4) + 2;
            WORD32 tmp_strd = MAX_CTB_SIZE + 4;
            WORD32 no_loop_filter_enabled = 0;
            ...
            /* Check the loop filter flags and copy the original values for back up */
            {
                UWORD32 u4_no_loop_filter_flag;
                WORD32 min_cu = 4;
                UWORD16 *pu2_src_tmp = pu2_src_chroma;

                for(i = 0; i < (sao_ht_chroma + min_cu - 1) / min_cu; i++)
                {
                    u4_no_loop_filter_flag = (*(UWORD32 *)(pu1_no_loop_filter_flag + i * loop_filter_strd)) >> ((((i4_ctb_x << log2_ctb_size) - sao_wd_chroma) / 8) % 8);
                    u4_no_loop_filter_flag &= (1 << ((sao_wd_chroma + (min_cu - 1)) / min_cu)) - 1;
            ...
            if(1 == ps_sao->b3_cb_type_idx)
            {
                ps_codec->s_func_selector.pf_hbd_sao_bo_chroma(pu2_src_chroma,
                        ...
                        pu2_src_left_chroma,
                        pu2_src_top_chroma,
                        (UWORD16 *)ps_sao_ctxt->pu1_sao_src_top_left_chroma_curr_ctb,
                        ps_sao->b5_cb_band_pos,
                        ps_sao->b5_cr_band_pos,
                        ai1_offset_cb,
                        ai1_offset_cr,
    ...
    if (i4_ctb_x > 0 && i4_ctb_y > 0)
    {
        ...
        if(ps_sao_ctxt->ps_slice_hdr->i1_slice_sao_luma_flag)
        {
            if(0 == ps_sao->b3_y_type_idx)
            {
                ...
                ihevc_memcpy((UWORD8 *)pu2_src_top_luma, (UWORD8 *)&pu2_src_luma[(sao_ht_luma - 1) * src_strd],
                ...
                /*In case of slices, unless we encounter multiple slice/tiled clips, don't enter*/
                {
                    if( (!ps_slice_hdr->i1_first_slice_in_pic_flag) || (ps_pps->i1_tiles_enabled_flag))
                    {
                        ...
                        if(!ps_slice_hdr->i1_first_slice_in_pic_flag)
                        {
                            ...
                            ihevc_memset_mul_8((UWORD8*)au4_idx_tl, 0, 8*sizeof(WORD32));
                        }
        ...
        if(0 != sao_wd_luma)
        {
            if(ps_sao_ctxt->ps_slice_hdr->i1_slice_sao_luma_flag)
            {
                ...
                else // if(2 <= ps_sao->b3_y_type_idx)
                {
                    ...
                    ihevc_memset_mul_8(au1_avail_luma, 255, 8);
                    ihevc_memset_mul_8(au1_tile_slice_boundary, 0, 8);
                    ihevc_memset_mul_8((UWORD8*)au4_idx_t,0,8*sizeof(WORD32));

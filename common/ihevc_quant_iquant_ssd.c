...
/*venkat:*/
WORD32 ihevc_quant_iquant_ssd_flat_scale_mat_smoothing
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
    ...
    )
{
    ...
    for(i = 0; i < i4_loop_size; i++)
    {
        for(j = 0; j < i4_loop_size; j++)
        {
            /*  Back up the coefficients before Quantization    */
            i2_temp = pi2_coeffs[j];
    ...
    *pi8_cost = ssd_cost;
    ...
    /* CSBF update */
    {
        ...
        for(block_row = 0; block_row < i4_loop_size; block_row += 4)
        {
            //block_col is incrementing by 1 for easy update of csbf pointer
            for(block_col = 0; block_col < i4_loop_size / 4; block_col++)
            {
                pi2_block = pi2_q_dst + block_row * dst_q_strd + block_col * 4;
                *(csbf + block_col) = 0;

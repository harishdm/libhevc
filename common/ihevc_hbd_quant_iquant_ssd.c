...
WORD32 ihevc_hbd_quant_iquant_ssd_flat_scale_mat
    (
    WORD16 *pi2_coeffs,
...
WORD32 ihevc_hbd_quant_iquant_flat_scale_mat
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_iquant_ssd_flat_scale_mat_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
...
WORD32 ihevc_hbd_quant_iquant_flat_scale_mat_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
...
WORD32 ihevc_hbd_quant_iquant_ssd
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
    ...
    for(i = 0; i < trans_size; i++)
    {
        for(j = 0; j < trans_size; j++)
        {
            ...
            QUANT_HBD
                ...
                pi2_q_dst[j],
                pi2_coeffs[j],
                pi2_quant_coeff[j] * g_ihevc_quant_scales[qp_rem],
                qp_div,
                log2_size,
                q_add,
                bit_depth
...
WORD32 ihevc_hbd_quant_iquant_ssd_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
...
WORD32 ihevc_hbd_quant_iquant_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
...
WORD32 ihevc_hbd_q_iq_ssd_var_rnd_fact
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
    ...
    )
{
    ...
    //fp_r0 = fopen("E:\\quant_average_r0.txt", "ab");
    //fp_r1 = fopen("E:\\quant_average_r1.txt", "ab");
    ...
    for(i = 0; i < trans_size; i++)
    {
        for(j = 0; j < trans_size; j++)
        {
            ...
            {
                /*  Quantization    */
                QUANT_HBD(pi2_q_dst[j],i2_temp,
                    pi2_quant_coeff[j] * g_ihevc_quant_scales[qp_rem], qp_div,
                    log2_size, 0, bit_depth);
                if (abs(pi2_q_dst[j]) >= 2)
                {
                    QUANT_HBD(pi2_q_dst[j],i2_temp,
                        pi2_quant_coeff[j] * g_ihevc_quant_scales[qp_rem], qp_div,
                        log2_size, ((1 << QUANT_ROUND_FACTOR_Q)/2), bit_depth);
                ...
                else if (abs(pi2_q_dst[j]) >= 1)
                {
                    QUANT_HBD(pi2_q_dst[j],i2_temp,
                        pi2_quant_coeff[j] * g_ihevc_quant_scales[qp_rem], qp_div,
                        log2_size, *pi4_quant_round_factor_1_2, bit_depth);
                }
                ...
                else
                {
                    ...
                    QUANT_HBD(pi2_q_dst[j],i2_temp,
                        pi2_quant_coeff[j] * g_ihevc_quant_scales[qp_rem], qp_div,
                        log2_size, *pi4_quant_round_factor_0_1, bit_depth);
                }
    ...
    //else
    //{
    //    printf("Quant_average %d \n", quant_average/(trans_size * trans_size));
    //}
    /* Store the cost */
    *pi8_cost = ssd_cost;
...
WORD32 ihevc_hbd_q_iq_var_rnd_fact
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
...
*******************************************************************************
*/
WORD32 ihevc_hbd_q_iq_ssd_flat_scale_mat_var_rnd_fact
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,
    ...
    )
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        for(j = 0; j < trans_size; j++)
        {
            ...
            {
                QUANT_NO_WEIGHTMAT_HBD(pi2_q_dst[j], pi2_coeffs[j],
                ...
                if (abs(pi2_q_dst[j]) >= 2)
                {
                    QUANT_NO_WEIGHTMAT_HBD(pi2_q_dst[j], i2_temp,
                        g_ihevc_quant_scales[qp_rem], qp_div,
                        log2_size, ((1 << QUANT_ROUND_FACTOR_Q)/2), bit_depth);
                }
                else if (abs(pi2_q_dst[j]) >= 1)
...
WORD32 ihevc_hbd_q_iq_flat_scale_mat_var_rnd_fact
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_q_dst,

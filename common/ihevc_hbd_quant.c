 ...
 *
 * @param[in] i4_csbf_strd
 *  coded sub block flag
 *
 * @param[out] pi4_zero_col
 *  zero column flag
 *
 * @param[out] pi4_zero_row
 *  zero column flag
 *
 * @returns  cbf
 * coded block flag
...
WORD32 ihevc_hbd_quant_4x4_ttype1(WORD16 *pi2_coeffs,
                              ...
                              UWORD8 u8_bit_depth)
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        ...
        {
            QUANT_HBD(pi2_dst[j], pi2_coeffs[j],
                  pi2_quant_coeff[j] * g_ihevc_quant_scales[i4_qp_rem], i4_qp_div,
                  log2_size, i4_q_add, u8_bit_depth);
        }

        pi2_coeffs += i4_src_strd;
        pi2_dst += i4_dst_strd;
    ...
    /* CSBF update */
    {
        ...
        {
            //block_col is incrementing by 1 for easy update of pi4_csbf pointer
            for(block_col = 0; block_col < trans_size / 4; block_col++)
            {
                pi2_block = pi2_dst + block_row * i4_dst_strd + block_col * 4;
                *(pi4_csbf + block_col) = 0;

                for(row = 0; row < 4; row++)
                {
                    for(col = 0; col < 4; col++)
                    {
                        if(pi2_block[row * i4_dst_strd + col] != 0)
                        {
                            *(pi4_csbf + block_col) = 1;
                            break;
                        }
                    }
                    if(*(pi4_csbf + block_col) == 1)
                    {
                        /* pi4_zero_col update *//* temp_zero_col = ~pi4_zero_col */
                        ...
                        // even if any 4x4 pi4_csbf is set

                        /* zero row update */ /* temp_zero_row = ~pi4_zero_row */
                ...
                cbf = cbf || (*(pi4_csbf + block_col)); // cbf update
            }
            pi4_csbf += i4_csbf_strd;
        }

        *pi4_zero_col = ~temp_zero_col; //final pi4_zero_col storing
        *pi4_zero_row = ~temp_zero_row; //final pi4_zero_row storing
    }
...
WORD32 ihevc_hbd_quant_4x4(WORD16 *pi2_coeffs,
                       WORD16 *pi2_quant_coeff,
                       WORD16 *pi2_dst,
                       WORD32 i4_qp_div,/* qpscaled / 6 */
                       WORD32 i4_qp_rem,/* qpscaled % 6 */
...
WORD32 ihevc_hbd_quant_8x8(WORD16 *pi2_coeffs,
                       WORD16 *pi2_quant_coeff,
                       WORD16 *pi2_dst,
                       WORD32 i4_qp_div,/* qpscaled / 6 */
                       WORD32 i4_qp_rem,/* qpscaled % 6 */
...
WORD32 ihevc_hbd_quant_16x16(WORD16 *pi2_coeffs,
                         WORD16 *pi2_quant_coeff,
                         WORD16 *pi2_dst,
                         WORD32 i4_qp_div,/* qpscaled / 6 */
                         WORD32 i4_qp_rem,/* qpscaled % 6 */
...
WORD32 ihevc_hbd_quant_32x32(WORD16 *pi2_coeffs,
                         WORD16 *pi2_quant_coeff,
                         WORD16 *pi2_dst,
                         WORD32 i4_qp_div,/* qpscaled / 6 */
                         WORD32 i4_qp_rem,/* qpscaled % 6 */
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_4x4
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
    ...
    )
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        for(j = 0; j < trans_size; j++)
        {
            QUANT_NO_WEIGHTMAT_HBD
                (
                pi2_dst[j],
                pi2_coeffs[j],
                g_ihevc_quant_scales[qp_rem],
                qp_div,
                log2_size,
                q_add,
                bit_depth
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_4x4_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_4x4_ttype1
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_4x4_ttype1_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_8x8
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_8x8_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
...
*******************************************************************************
*/
WORD32 ihevc_hbd_quant_flat_scale_mat_16x16
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_16x16_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_32x32
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,
 ...
 *******************************************************************************
 */
WORD32 ihevc_hbd_quant_flat_scale_mat_32x32_rdoq
    (
    WORD16 *pi2_coeffs,
    WORD16 *pi2_quant_coeff,
    WORD16 *pi2_dst,

/******************************************************************************
*
* Copyright (C) 2012 Ittiam Systems Pvt Ltd, Bangalore
*
* Licensed under the Apache License, Version 2.0 (the "License");
* you may not use this file except in compliance with the License.
* You may obtain a copy of the License at:
*
* http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*
******************************************************************************/
/**
 *******************************************************************************
 * @file
 *  ihevc_iquant_itrans_recon_16x16.c
 *
 * @brief
 *  Contains function definitions for inverse  quantization, inverse
 * transform and reconstruction 32x32
 *
 * @author
 *  Ittiam
 *
 * @par List of Functions:
 *  - ihevc_iquant_itrans_recon_16x16()
 *
 * @remarks
 *  None
 *
 *******************************************************************************
 */
#include <stdio.h>
#include <string.h>
#include "ihevc_typedefs.h"
#include "ihevc_macros.h"
#include "ihevc_platform_macros.h"
#include "ihevc_defs.h"
#include "ihevc_trans_tables.h"
#include "ihevc_iquant_itrans_recon.h"
#include "ihevc_func_selector.h"
#include "ihevc_trans_macros.h"

/**
 *******************************************************************************
 *
 * @brief
 *  This function performs inverse quantization, inverse  transform and
 * reconstruction for 16x16 input block
 *
 * @par Description:
 *  Performs inverse quantization , inverse transform  and adds the
 * prediction data and clips output to 8 bit
 *
 * @param[in] pi2_src
 *  Input 16x16 coefficients
 *
 * @param[in] pi2_tmp
 *  Temporary 16x16 buffer for storing inverse
 *  transform 1st stage output
 *
 * @param[in] pu2_pred
 *  Prediction 16x16 block
 *
 * @param[in] pi2_dequant_coeff
 *  Dequant Coeffs
 *
 * @param[out] pu2_dst
 *  Output 16x16 block
 *
 * @param[in] i4_qp_div
 *  Quantization parameter / 6
 *
 * @param[in] i4_qp_rem
 *  Quantization parameter % 6
 *
 * @param[in] i4_src_strd
 *  Input stride
 *
 * @param[in] i4_pred_strd
 *  Prediction stride
 *
 * @param[in] i4_dst_strd
 *  Output Stride
 *
 * @param[in] i4_zero_cols
 *  Zero columns in pi2_src
 *
 * @param[in] i4_zero_rows
 *  Zero Rows in pi2_src
 *
 * @returns  Void
 *
 * @remarks
 *  None
 *
 *******************************************************************************
 */
void ihevc_hbd_iquant_itrans_recon_16x16(WORD16 *pi2_src,
                                     WORD16 *pi2_tmp,
                                     UWORD16 *pu2_pred,
                                     WORD16 *pi2_dequant_coeff,
                                     UWORD16 *pu2_dst,
                                     WORD32 i4_qp_div,/* qpscaled / 6 */
                                     WORD32 i4_qp_rem,/* qpscaled % 6 */
                                     WORD32 i4_src_strd,
                                     WORD32 i4_pred_strd,
                                     WORD32 i4_dst_strd,
                                     WORD32 i4_zero_cols,
                                     WORD32 i4_zero_rows,
                                     UWORD8 u1_bit_depth)

{
    /* Inverse Transform */
    WORD32 j, k;
    WORD32 e[8], o[8];
    WORD32 ee[4], eo[4];
    WORD32 eee[2], eeo[2];
    WORD32 add;
    WORD32 shift;
    WORD16 *pi2_tmp_orig;
    WORD32 shift_iq;
    WORD16 clip_limit;
    WORD32 trans_size = TRANS_SIZE_16;
    WORD32 zero_rows_2nd_stage = i4_zero_cols;
    WORD32 row_limit_2nd_stage;

    if((i4_zero_cols & 0xFFF0) == 0xFFF0)
        row_limit_2nd_stage = 4;
    else if((i4_zero_cols & 0xFF00) == 0xFF00)
        row_limit_2nd_stage = 8;
    else
        row_limit_2nd_stage = TRANS_SIZE_16;

    clip_limit = (1 << u1_bit_depth) -1;
    /* Inverse Quantization constants */
    {
        WORD32 log2_trans_size;

        log2_trans_size = 4;
        shift_iq = u1_bit_depth + log2_trans_size - 5;
    }

    pi2_tmp_orig = pi2_tmp;

    if((i4_zero_rows & 0xFFF0) == 0xFFF0)  /* Only First 4 rows of input are non-zero */
    {
        /************************************************************************************************/
        /**********************************START - IT_RECON_16x16****************************************/
        /************************************************************************************************/
        /* Inverse Transform 1st stage */
        shift = IT_SHIFT_STAGE_1;
        add = 1 << (shift - 1);

        for(j = 0; j < row_limit_2nd_stage; j++)
        {
            /* Checking for Zero Cols */
            if((i4_zero_cols & 1) == 1)
            {
                memset(pi2_tmp, 0, trans_size * sizeof(WORD16));
            }
            else
            {
                WORD32 iq_tmp_1, iq_tmp_2;

                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[1*i4_src_strd],
                           pi2_dequant_coeff[1*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_2,
                           pi2_src[3*i4_src_strd],
                           pi2_dequant_coeff[3*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    o[k] = g_ai2_ihevc_trans_16[1][k] * iq_tmp_1
                                    + g_ai2_ihevc_trans_16[3][k] * iq_tmp_2;
                }
                for(k = 0; k < 4; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[2*i4_src_strd],
                           pi2_dequant_coeff[2*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    eo[k] = g_ai2_ihevc_trans_16[2][k] * iq_tmp_1;
                }

                eeo[0] = 0;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eee[0] = g_ai2_ihevc_trans_16[0][0] * iq_tmp_1;

                eeo[1] = 0;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eee[1] = g_ai2_ihevc_trans_16[0][1] * iq_tmp_1;

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    pi2_tmp[k] =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift) );
                    pi2_tmp[k + 8] =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift) );
                }
            }
            pi2_src++;
            pi2_dequant_coeff++;
            pi2_tmp += trans_size;

            i4_zero_cols = i4_zero_cols >> 1;
        }

        pi2_tmp = pi2_tmp_orig;

        /* Inverse Transform 2nd stage */
        shift = 20 - u1_bit_depth;
        add = 1 << (shift - 1);

        if((zero_rows_2nd_stage & 0xFFF0) == 0xFFF0) /* Only First 4 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size];
                }
                eeo[0] = 0;
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = 0;
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else if((zero_rows_2nd_stage & 0xFF00) == 0xFF00) /* First 4 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else    /* All rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size]
                                    + g_ai2_ihevc_trans_16[9][k]
                                                    * pi2_tmp[9 * trans_size]
                                    + g_ai2_ihevc_trans_16[11][k]
                                                    * pi2_tmp[11 * trans_size]
                                    + g_ai2_ihevc_trans_16[13][k]
                                                    * pi2_tmp[13 * trans_size]
                                    + g_ai2_ihevc_trans_16[15][k]
                                                    * pi2_tmp[15 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size]
                                    + g_ai2_ihevc_trans_16[10][k]
                                                    * pi2_tmp[10 * trans_size]
                                    + g_ai2_ihevc_trans_16[14][k]
                                                    * pi2_tmp[14 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][0]
                                                * pi2_tmp[12 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][0]
                                                * pi2_tmp[8 * trans_size];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][1]
                                                * pi2_tmp[12 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][1]
                                                * pi2_tmp[8 * trans_size];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        /************************************************************************************************/
        /************************************END - IT_RECON_16x16****************************************/
        /************************************************************************************************/
    }
    else if((i4_zero_rows & 0xFF00) == 0xFF00)  /* First 8 rows of input are non-zero */
    {
        /************************************************************************************************/
        /**********************************START - IT_RECON_16x16****************************************/
        /************************************************************************************************/

        /* Inverse Transform 1st stage */
        shift = IT_SHIFT_STAGE_1;
        add = 1 << (shift - 1);

        for(j = 0; j < row_limit_2nd_stage; j++)
        {
            /* Checking for Zero Cols */
            if((i4_zero_cols & 1) == 1)
            {
                memset(pi2_tmp, 0, trans_size * sizeof(WORD16));
            }
            else
            {
                WORD32 iq_tmp_1, iq_tmp_2, iq_tmp_3, iq_tmp_4;

                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[1*i4_src_strd],
                           pi2_dequant_coeff[1*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_2,
                           pi2_src[3*i4_src_strd],
                           pi2_dequant_coeff[3*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_3,
                           pi2_src[5*i4_src_strd],
                           pi2_dequant_coeff[5*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_4,
                           pi2_src[7*i4_src_strd],
                           pi2_dequant_coeff[7*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    o[k] = g_ai2_ihevc_trans_16[1][k] * iq_tmp_1
                                    + g_ai2_ihevc_trans_16[3][k] * iq_tmp_2
                                    + g_ai2_ihevc_trans_16[5][k] * iq_tmp_3
                                    + g_ai2_ihevc_trans_16[7][k] * iq_tmp_4;
                }
                for(k = 0; k < 4; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[2*i4_src_strd],
                           pi2_dequant_coeff[2*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_2,
                           pi2_src[6*i4_src_strd],
                           pi2_dequant_coeff[6*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    eo[k] = g_ai2_ihevc_trans_16[2][k] * iq_tmp_1
                                    + g_ai2_ihevc_trans_16[6][k] * iq_tmp_2;
                }

                IQUANT(iq_tmp_1,
                       pi2_src[4*i4_src_strd],
                       pi2_dequant_coeff[4*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * iq_tmp_1;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eee[0] = g_ai2_ihevc_trans_16[0][0] * iq_tmp_1;

                IQUANT(iq_tmp_1,
                       pi2_src[4*i4_src_strd],
                       pi2_dequant_coeff[4*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * iq_tmp_1;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                eee[1] = g_ai2_ihevc_trans_16[0][1] * iq_tmp_1;

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    pi2_tmp[k] =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift) );
                    pi2_tmp[k + 8] =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift) );
                }
            }
            pi2_src++;
            pi2_dequant_coeff++;
            pi2_tmp += trans_size;

            i4_zero_cols = i4_zero_cols >> 1;
        }

        pi2_tmp = pi2_tmp_orig;

        /* Inverse Transform 2nd stage */
        shift = 20 - u1_bit_depth;
        add = 1 << (shift - 1);

        if((zero_rows_2nd_stage & 0xFFF0) == 0xFFF0) /* First 4 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size];
                }
                eeo[0] = 0;
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = 0;
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else if((zero_rows_2nd_stage & 0xFF00) == 0xFF00) /* First 4 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else /* All rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size]
                                    + g_ai2_ihevc_trans_16[9][k]
                                                    * pi2_tmp[9 * trans_size]
                                    + g_ai2_ihevc_trans_16[11][k]
                                                    * pi2_tmp[11 * trans_size]
                                    + g_ai2_ihevc_trans_16[13][k]
                                                    * pi2_tmp[13 * trans_size]
                                    + g_ai2_ihevc_trans_16[15][k]
                                                    * pi2_tmp[15 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size]
                                    + g_ai2_ihevc_trans_16[10][k]
                                                    * pi2_tmp[10 * trans_size]
                                    + g_ai2_ihevc_trans_16[14][k]
                                                    * pi2_tmp[14 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][0]
                                                * pi2_tmp[12 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][0]
                                                * pi2_tmp[8 * trans_size];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][1]
                                                * pi2_tmp[12 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][1]
                                                * pi2_tmp[8 * trans_size];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        /************************************************************************************************/
        /************************************END - IT_RECON_16x16****************************************/
        /************************************************************************************************/
    }
    else  /* All rows of input are non-zero */
    {
        /************************************************************************************************/
        /**********************************START - IT_RECON_16x16****************************************/
        /************************************************************************************************/

        /* Inverse Transform 1st stage */
        shift = IT_SHIFT_STAGE_1;
        add = 1 << (shift - 1);

        for(j = 0; j < row_limit_2nd_stage; j++)
        {
            /* Checking for Zero Cols */
            if((i4_zero_cols & 1) == 1)
            {
                memset(pi2_tmp, 0, trans_size * sizeof(WORD16));
            }
            else
            {
                WORD32 iq_tmp_1, iq_tmp_2, iq_tmp_3, iq_tmp_4, iq_tmp_5,
                                iq_tmp_6, iq_tmp_7, iq_tmp_8;

                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[1*i4_src_strd],
                           pi2_dequant_coeff[1*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_2,
                           pi2_src[3*i4_src_strd],
                           pi2_dequant_coeff[3*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_3,
                           pi2_src[5*i4_src_strd],
                           pi2_dequant_coeff[5*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_4,
                           pi2_src[7*i4_src_strd],
                           pi2_dequant_coeff[7*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_5,
                           pi2_src[9*i4_src_strd],
                           pi2_dequant_coeff[9*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_6,
                           pi2_src[11*i4_src_strd],
                           pi2_dequant_coeff[11*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_7,
                           pi2_src[13*i4_src_strd],
                           pi2_dequant_coeff[13*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_8,
                           pi2_src[15*i4_src_strd],
                           pi2_dequant_coeff[15*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    o[k] = g_ai2_ihevc_trans_16[1][k] * iq_tmp_1
                                    + g_ai2_ihevc_trans_16[3][k] * iq_tmp_2
                                    + g_ai2_ihevc_trans_16[5][k] * iq_tmp_3
                                    + g_ai2_ihevc_trans_16[7][k] * iq_tmp_4
                                    + g_ai2_ihevc_trans_16[9][k] * iq_tmp_5
                                    + g_ai2_ihevc_trans_16[11][k] * iq_tmp_6
                                    + g_ai2_ihevc_trans_16[13][k] * iq_tmp_7
                                    + g_ai2_ihevc_trans_16[15][k] * iq_tmp_8;
                }
                for(k = 0; k < 4; k++)
                {
                    IQUANT(iq_tmp_1,
                           pi2_src[2*i4_src_strd],
                           pi2_dequant_coeff[2*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_2,
                           pi2_src[6*i4_src_strd],
                           pi2_dequant_coeff[6*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_3,
                           pi2_src[10*i4_src_strd],
                           pi2_dequant_coeff[10*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);
                    IQUANT(iq_tmp_4,
                           pi2_src[14*i4_src_strd],
                           pi2_dequant_coeff[14*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                           shift_iq, i4_qp_div);

                    eo[k] = g_ai2_ihevc_trans_16[2][k] * iq_tmp_1
                                    + g_ai2_ihevc_trans_16[6][k] * iq_tmp_2
                                    + g_ai2_ihevc_trans_16[10][k] * iq_tmp_3
                                    + g_ai2_ihevc_trans_16[14][k] * iq_tmp_4;
                }

                IQUANT(iq_tmp_1,
                       pi2_src[4*i4_src_strd],
                       pi2_dequant_coeff[4*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                IQUANT(iq_tmp_2,
                       pi2_src[12*i4_src_strd],
                       pi2_dequant_coeff[12*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);

                eeo[0] = g_ai2_ihevc_trans_16[4][0] * iq_tmp_1
                                + g_ai2_ihevc_trans_16[12][0] * iq_tmp_2;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                IQUANT(iq_tmp_2,
                       pi2_src[8*i4_src_strd],
                       pi2_dequant_coeff[8*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);

                eee[0] = g_ai2_ihevc_trans_16[0][0] * iq_tmp_1
                                + g_ai2_ihevc_trans_16[8][0] * iq_tmp_2;

                IQUANT(iq_tmp_1,
                       pi2_src[4*i4_src_strd],
                       pi2_dequant_coeff[4*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                IQUANT(iq_tmp_2,
                       pi2_src[12*i4_src_strd],
                       pi2_dequant_coeff[12*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);

                eeo[1] = g_ai2_ihevc_trans_16[4][1] * iq_tmp_1
                                + g_ai2_ihevc_trans_16[12][1] * iq_tmp_2;

                IQUANT(iq_tmp_1,
                       pi2_src[0*i4_src_strd],
                       pi2_dequant_coeff[0*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);
                IQUANT(iq_tmp_2,
                       pi2_src[8*i4_src_strd],
                       pi2_dequant_coeff[8*trans_size] *g_ihevc_iquant_scales[i4_qp_rem],
                       shift_iq, i4_qp_div);

                eee[1] = g_ai2_ihevc_trans_16[0][1] * iq_tmp_1
                                + g_ai2_ihevc_trans_16[8][1] * iq_tmp_2;

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    pi2_tmp[k] =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift) );
                    pi2_tmp[k + 8] =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift) );
                }
            }
            pi2_src++;
            pi2_dequant_coeff++;
            pi2_tmp += trans_size;

            i4_zero_cols = i4_zero_cols >> 1;
        }

        pi2_tmp = pi2_tmp_orig;

        /* Inverse Transform 2nd stage */
        shift = 20 - u1_bit_depth;
        add = 1 << (shift - 1);

        if((zero_rows_2nd_stage & 0xFFF0) == 0xFFF0) /* First 4 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size];
                }
                eeo[0] = 0;
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = 0;
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else if((zero_rows_2nd_stage & 0xFF00) == 0xFF00) /* First 8 rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        else /* All rows of output of 1st stage are non-zero */
        {
            for(j = 0; j < trans_size; j++)
            {
                WORD32 itrans_out;
                /* Utilizing symmetry properties to the maximum to minimize the number of multiplications */
                for(k = 0; k < 8; k++)
                {
                    o[k] = g_ai2_ihevc_trans_16[1][k] * pi2_tmp[trans_size]
                                    + g_ai2_ihevc_trans_16[3][k]
                                                    * pi2_tmp[3 * trans_size]
                                    + g_ai2_ihevc_trans_16[5][k]
                                                    * pi2_tmp[5 * trans_size]
                                    + g_ai2_ihevc_trans_16[7][k]
                                                    * pi2_tmp[7 * trans_size]
                                    + g_ai2_ihevc_trans_16[9][k]
                                                    * pi2_tmp[9 * trans_size]
                                    + g_ai2_ihevc_trans_16[11][k]
                                                    * pi2_tmp[11 * trans_size]
                                    + g_ai2_ihevc_trans_16[13][k]
                                                    * pi2_tmp[13 * trans_size]
                                    + g_ai2_ihevc_trans_16[15][k]
                                                    * pi2_tmp[15 * trans_size];
                }
                for(k = 0; k < 4; k++)
                {
                    eo[k] = g_ai2_ihevc_trans_16[2][k] * pi2_tmp[2 * trans_size]
                                    + g_ai2_ihevc_trans_16[6][k]
                                                    * pi2_tmp[6 * trans_size]
                                    + g_ai2_ihevc_trans_16[10][k]
                                                    * pi2_tmp[10 * trans_size]
                                    + g_ai2_ihevc_trans_16[14][k]
                                                    * pi2_tmp[14 * trans_size];
                }
                eeo[0] = g_ai2_ihevc_trans_16[4][0] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][0]
                                                * pi2_tmp[12 * trans_size];
                eee[0] = g_ai2_ihevc_trans_16[0][0] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][0]
                                                * pi2_tmp[8 * trans_size];
                eeo[1] = g_ai2_ihevc_trans_16[4][1] * pi2_tmp[4 * trans_size]
                                + g_ai2_ihevc_trans_16[12][1]
                                                * pi2_tmp[12 * trans_size];
                eee[1] = g_ai2_ihevc_trans_16[0][1] * pi2_tmp[0]
                                + g_ai2_ihevc_trans_16[8][1]
                                                * pi2_tmp[8 * trans_size];

                /* Combining e and o terms at each hierarchy levels to calculate the final spatial domain vector */
                for(k = 0; k < 2; k++)
                {
                    ee[k] = eee[k] + eeo[k];
                    ee[k + 2] = eee[1 - k] - eeo[1 - k];
                }
                for(k = 0; k < 4; k++)
                {
                    e[k] = ee[k] + eo[k];
                    e[k + 4] = ee[3 - k] - eo[3 - k];
                }
                for(k = 0; k < 8; k++)
                {
                    itrans_out =
                                    CLIP_S16( ((e[k] + o[k] + add)>>shift ) );
                    pu2_dst[k] = CLIP3((itrans_out + pu2_pred[k]), 0, clip_limit );

                    itrans_out =
                                    CLIP_S16( ((e[7-k] - o[7-k] + add)>>shift ) );
                    pu2_dst[k + 8] = CLIP3((itrans_out + pu2_pred[k+8]), 0, clip_limit );
                }
                pi2_tmp++;
                pu2_pred += i4_pred_strd;
                pu2_dst += i4_dst_strd;
            }
        }
        /************************************************************************************************/
        /************************************END - IT_RECON_16x16****************************************/
        /************************************************************************************************/
    }
}

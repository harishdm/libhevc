 ...
 *******************************************************************************
 */
void ihevc_hbd_resi_trans_quant_4x4_ttype1(UWORD16 *pu2_src,
                                       UWORD16 *pu2_pred,
                                       WORD16 *pi2_tmp,
                                       ...
                                       UWORD8 u1_bit_depth)
{
    ...
    {
        WORD16 resi_tmp_1, resi_tmp_2, resi_tmp_3;
        ...
        pi2_tmp[0] = (29 * c[0] + 55 * c[1] + c[3] + add) >> shift;
        ...
        pi2_tmp[trans_size] =
                        (74 * (resi_tmp_1 + resi_tmp_2 - resi_tmp_3) + add)
                                        >> shift;
        pi2_tmp[2 * trans_size] = (29 * c[2] + 55 * c[0] - c[3] + add) >> shift;
        pi2_tmp[3 * trans_size] = (55 * c[2] - 29 * c[1] + c[3] + add) >> shift;
    ...
    /* Forward transform 2nd stage and Quantization */
    {
        ...
        for(i = 0; i < TRANS_SIZE_4; i++)
        {
            WORD32 trans_out;
            // Intermediate Variables
            c[0] = pi2_tmp[0] + pi2_tmp[3];
            c[1] = pi2_tmp[1] + pi2_tmp[3];
            c[2] = pi2_tmp[0] - pi2_tmp[1];
            c[3] = 74 * pi2_tmp[2];
            ...
            trans_out = ((74 * (pi2_tmp[0] + pi2_tmp[1] - pi2_tmp[3]) + add)
                            >> shift);
 ...
 *******************************************************************************
 */
void ihevc_hbd_resi_trans_quant_4x4(UWORD16 *pu2_src,
                                UWORD16 *pu2_pred,
                                WORD16 *pi2_tmp,
                                WORD16 *pi2_quant_coeff,
                                WORD16 *pi2_dst,
                                ...
                                UWORD8 u1_bit_depth)
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        ...
        resi_tmp_1 = pu2_src[1] - pu2_pred[1];
        resi_tmp_2 = pu2_src[2] - pu2_pred[2];
 ...
 *******************************************************************************
 */
void ihevc_hbd_resi_trans_quant_8x8(UWORD16 *pu2_src,
                                UWORD16 *pu2_pred,
                                WORD16 *pi2_tmp,
                                WORD16 *pi2_quant_coeff,
                                WORD16 *pi2_dst,
                                ...
                                UWORD8 u1_bit_depth)
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        ...
        for(k = 0; k < 4; k++)
        {
            ...
            resi_tmp_2 = pu2_src[7 - k] - pu2_pred[7 - k];
 ...
 *******************************************************************************
 */
void ihevc_hbd_resi_trans_quant_16x16(UWORD16 *pu2_src,
                                  UWORD16 *pu2_pred,
                                  WORD16 *pi2_tmp,
                                  WORD16 *pi2_quant_coeff,
                                  WORD16 *pi2_dst,
                                  ...
                                  UWORD8 u1_bit_depth)
{
    ...
    for(i = 0; i < trans_size; i++)
    {
        ...
        {
            resi_tmp_1 = pu2_src[k] - pu2_pred[k];
            resi_tmp_2 = pu2_src[15 - k] - pu2_pred[15 - k];
            e[k] = resi_tmp_1 + resi_tmp_2;
            o[k] = resi_tmp_1 - resi_tmp_2;
        }
        /* ee and eo */
 ...
 *******************************************************************************
 */
void ihevc_hbd_resi_trans_quant_32x32(UWORD16 *pu2_src,
                                  UWORD16 *pu2_pred,
                                  WORD16 *pi2_tmp,
                                  WORD16 *pi2_quant_coeff,
                                  WORD16 *pi2_dst,
    ...
    /* Forward transform 2nd stage and Quantization */
    {
        ...
        for(i = 0; i < TRANS_SIZE_32; i++)
        {
            ...
            trans_out = (g_ai2_ihevc_trans_32[16][0] * eeee[0]
                            ...
                            >> shift;
            QUANT_HBD(pi2_dst[16*i4_dst_strd], trans_out,
                  pi2_quant_coeff[16*trans_size]*g_ihevc_quant_scales[i4_qp_rem],
                  i4_qp_div, log2_trans_size, i4_q_add, u1_bit_depth);
            ...
            trans_out = (g_ai2_ihevc_trans_32[24][0] * eeeo[0]
                            ...
                            >> shift;
            QUANT_HBD(pi2_dst[24*i4_dst_strd], trans_out,
                  pi2_quant_coeff[24*trans_size]*g_ihevc_quant_scales[i4_qp_rem],
                  i4_qp_div, log2_trans_size, i4_q_add, u1_bit_depth);
